...
export function StockManager({ onStocksUpdate }: StockManagerProps) {
  ...
  const addStock = () => {
    ...
    if (stockSymbol && !selectedStocks.includes(stockSymbol) && selectedStocks.length < 3) {
  ...
  const removeStock = (stockSymbol: string) => {
    ...
    setSelectedStocks(newStocks);
    setStockData(prev => prev.filter(stock => stock.ticker !== stockSymbol));
    localStorage.setItem("userStocks", JSON.stringify(newStocks));
    onStocksUpdate(newStocks);
  };
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            {/* Current Stocks */}
            <div>
              <h4 className="text-sm font-medium mb-2">Your Stocks ({selectedStocks.length}/3)</h4>
