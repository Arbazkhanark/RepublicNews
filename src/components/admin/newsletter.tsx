...
// import { Mail, Users, Send, Plus, MoreHorizontal, Edit, Trash2, Eye, TrendingUp, Download, Upload } from "lucide-react"
...
//   {
//     id: 2,
//     subject: "Breaking: Economic Policy Updates",
//     status: "sent",
//     sentAt: "2024-01-12 14:30",
//     recipients: 12300,
//     openRate: 72.1,
//     clickRate: 15.2,
//   },
//   {
//     id: 3,
//     subject: "Sports Championship Special Edition",
//     status: "draft",
//     sentAt: null,
//     recipients: 0,
//     openRate: 0,
...
//     console.log("Sending newsletter:", emailForm)
//     setIsComposeDialogOpen(false)
//     setEmailForm({ subject: "", content: "", template: "default" })
//   }
...
//           <p className="text-muted-foreground">Manage subscribers and send newsletters to your audience</p>
//         </div>
...
//                     onValueChange={(value) => setEmailForm({ ...emailForm, template: value })}
//                   >
//                     <SelectTrigger>
//                       <SelectValue />
//                     </SelectTrigger>
//                     <SelectContent>
//                       <SelectItem value="default">Default Newsletter</SelectItem>
...
//                   <span className="text-sm">This will be sent to 12,847 active subscribers</span>
//                 </div>
//               </div>
//               <DialogFooter>
...
//                         <div>{campaign.openRate}% open</div>
//                         <div>{campaign.clickRate}% click</div>
//                       </div>
//                     )}
//                   </div>
//                 ))}
...
//                     <span className="font-medium">8,234 (64%)</span>
//                   </div>
...
//                     <span className="font-medium">4,613 (36%)</span>
//                   </div>
//                   <div className="w-full bg-secondary rounded-full h-2">
...
//                     <span className="font-medium">7,892 (61%)</span>
//                   </div>
//                   <div className="flex justify-between text-sm">
//                     <span>Social Media</span>
//                     <span className="font-medium">3,456 (27%)</span>
//                   </div>
//                   <div className="flex justify-between text-sm">
//                     <span>Referrals</span>
//                     <span className="font-medium">1,499 (12%)</span>
//                   </div>
//                 </div>
//               </CardContent>
//             </Card>
...
//                         onValueChange={(value) => setSubscriberForm({ ...subscriberForm, language: value })}
//                       >
//                         <SelectTrigger>
//                           <SelectValue />
//                         </SelectTrigger>
...
//                       </TableCell>
//                       <TableCell>
//                         <Badge variant="outline">{subscriber.language === "en" ? "English" : "Hindi"}</Badge>
//                       </TableCell>
...
//                       <TableCell>{campaign.sentAt || "Not sent"}</TableCell>
//                       <TableCell className="text-right">
//                         <DropdownMenu>
//                           <DropdownMenuTrigger asChild>
//                             <Button variant="ghost" className="h-8 w-8 p-0">
...
//                 <div className="text-sm font-medium text-muted-foreground mb-3">Email Content:</div>
...
//     {
//       title: "Active Campaigns",
//       value: mockCampaigns.filter(c => c.status === "sent" || c.status === "scheduled").length.toString(),
//       change: "+1",
//       trend: "up",
//       icon: Mail,
//     },
//     {
//       title: "Open Rate",
//       value: mockCampaigns.length > 0 
//         ? `${(mockCampaigns.reduce((sum, c) => sum + c.opens, 0) / mockCampaigns.reduce((sum, c) => sum + c.recipients, 1) * 100).toFixed(1)}%`
//         : "0%",
//       change: "+2.3%",
//       trend: "up",
//       icon: Eye,
...
//       title: "Click Rate",
//       value: mockCampaigns.length > 0 
//         ? `${(mockCampaigns.reduce((sum, c) => sum + c.clicks, 0) / mockCampaigns.reduce((sum, c) => sum + c.recipients, 1) * 100).toFixed(1)}%`
//         : "0%",
//       change: "+1.1%",
//       trend: "up",
//       icon: TrendingUp,
...
//     console.log("Sending newsletter:", emailForm)
//     setIsComposeDialogOpen(false)
//     setEmailForm({ subject: "", content: "", template: "default", language: "en", status: "draft", scheduledAt: "" })
...
//                     onValueChange={(value) => setEmailForm({ ...emailForm, status: value as "draft" | "scheduled" | "sent" | "failed" })}
//                   >
//                     <SelectTrigger>
//                       <SelectValue />
//                     </SelectTrigger>
...
//                 <div className="flex items-center space-x-2 p-3 bg-muted rounded-lg">
//                   <Users className="h-4 w-4" />
//                   <span className="text-sm">This will be sent to {subscribers.filter(s => s.isActive).length} active subscribers</span>
//                 </div>
//               </div>
//               <DialogFooter>
//                 <Button variant="outline" onClick={() => setIsComposeDialogOpen(false)}>
...
//                         {campaign.sentAt && <span>{formatDate(campaign.sentAt)}</span>}
//                       </div>
//                     </div>
//                     {campaign.status === "sent" && (
//                       <div className="text-right text-xs">
//                         <div>{((campaign.opens / campaign.recipients) * 100).toFixed(1)}% open</div>
//                         <div>{((campaign.clicks / campaign.recipients) * 100).toFixed(1)}% click</div>
//                       </div>
//                     )}
//                   </div>
//                 ))}
...
//                       {subscribers.length > 0 ? Math.round((subscribers.filter(s => s.language === "en").length / subscribers.length) * 100) : 0}%)
//                     </span>
//                   </div>
//                   <div className="w-full bg-secondary rounded-full h-2">
//                     <div 
...
//                       style={{ 
//                         width: `${subscribers.length > 0 ? (subscribers.filter(s => s.language === "en").length / subscribers.length) * 100 : 0}%` 
//                       }} 
//                     />
//                   </div>
//                 </div>
...
//                       {subscribers.length > 0 ? Math.round((subscribers.filter(s => s.language === "hi").length / subscribers.length) * 100) : 0}%)
//                     </span>
//                   </div>
//                   <div className="w-full bg-secondary rounded-full h-2">
//                     <div 
//                       className="bg-primary h-2 rounded-full" 
//                       style={{ 
//                         width: `${subscribers.length > 0 ? (subscribers.filter(s => s.language === "hi").length / subscribers.length) * 100 : 0}%` 
//                       }} 
//                     />
//                   </div>
//                 </div>
...
//                       {subscribers.length > 0 ? Math.round((subscribers.filter(s => s.isActive).length / subscribers.length) * 100) : 0}%)
//                     </span>
//                   </div>
//                   <div className="flex justify-between text-sm">
...
//                       {subscribers.length > 0 ? Math.round((subscribers.filter(s => s.isVerified).length / subscribers.length) * 100) : 0}%)
//                     </span>
//                   </div>
//                 </div>
//               </CardContent>
...
//                         }
//                       </TableCell>
//                       <TableCell>{campaign.sentAt ? formatDate(campaign.sentAt) : "Not sent"}</TableCell>
//                       <TableCell className="text-right">
//                         <DropdownMenu>
//                           <DropdownMenuTrigger asChild>
//                             <Button variant="ghost" className="h-8 w-8 p-0">
...
//                                   Edit
//                                 </DropdownMenuItem>
//                                 <DropdownMenuItem onClick={() => handleSendCampaign(campaign._id)}>
//                                   <Send className="mr-2 h-4 w-4" />
//                                   Send Now
//                                 </DropdownMenuItem>
//                               </>
...
// } from "@/components/ui/dropdown-menu"
// import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
// import { Mail, Users, Send, Plus, MoreHorizontal, Edit, Trash2, Eye, TrendingUp, Download, Upload, Calendar, Clock } from "lucide-react"
// import { toast } from "sonner"
...
//   const AUTH_TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2OGJjOGFkNzE0ZjA1ZTlkYmM1YTQ1ZjQiLCJlbWFpbCI6ImFyYmFhYXpraGFuYXJrMjNAZ21haWwuY29tIiwicm9sZSI6ImFkbWluIiwibmFtZSI6IkFyYmF6IEtoYW4iLCJpYXQiOjE3NTgxMjY4NzMsImV4cCI6MTc1ODczMTY3M30.9WP3_AmSr9kyld6Bv9npGEWRRpPvrC3hBEe87OL5PeQ"
...
//       setCampaigns(campaigns.map(campaign => 
//         campaign._id === id ? {...campaign, status: "sent", sentAt: new Date().toISOString()} : campaign
//       ))
...
//     {
//       title: "Active Campaigns",
//       value: campaigns.filter(c => c.status === "sent" || c.status === "scheduled").length.toString(),
//       change: "+1",
//       trend: "up",
//       icon: Mail,
//     },
...
//         ? `${(campaigns.reduce((sum, c) => sum + c.opens, 0) / campaigns.reduce((sum, c) => sum + c.recipients, 1) * 100).toFixed(1)}%`
//         : "0%",
//       change: "+2.3%",
//       trend: "up",
//       icon: Eye,
...
//       title: "Click Rate",
//       value: campaigns.length > 0 
//         ? `${(campaigns.reduce((sum, c) => sum + c.clicks, 0) / campaigns.reduce((sum, c) => sum + c.recipients, 1) * 100).toFixed(1)}%`
//         : "0%",
//       change: "+1.1%",
//       trend: "up",
//       icon: TrendingUp,
...
//         scheduledAt: emailForm.status === "scheduled" ? emailForm.scheduledAt : undefined,
//         recipients: 0,
//         opens: 0,
//         clicks: 0,
...
//             <DialogContent className="max-w-2xl">
//               <DialogHeader>
//                 <DialogTitle>{editingCampaign ? "Edit Campaign" : "Compose Newsletter"}</DialogTitle>
//                 <DialogDescription>
//                   {editingCampaign ? "Edit your campaign details." : "Create and send a newsletter to your subscribers."}
//                 </DialogDescription>
//               </DialogHeader>
//               <div className="space-y-4 scroll-auto overflow-y-auto max-h-[60vh] pr-2">
//                 <div>
...
//                     onValueChange={(value) => setEmailForm({ ...emailForm, language: value })}
//                   >
//                     <SelectTrigger>
//                       <SelectValue />
//                     </SelectTrigger>
...
//                       id="scheduledAt"
//                       type="datetime-local"
//                       value={emailForm.scheduledAt ? format(new Date(emailForm.scheduledAt), "yyyy-MM-dd'T'HH:mm") : ""}
//                       onChange={(e) => setEmailForm({ ...emailForm, scheduledAt: new Date(e.target.value).toISOString() })}
//                     />
//                   </div>
//                 )}
//                 <div className="flex items-center space-x-2 p-3 bg-muted rounded-lg">
...
//                             <span>{format(new Date(campaign.scheduledAt), "MMM d, h:mm a")}</span>
//                           </div>
//                         )}
//                       </div>
//                     </div>
//                     {campaign.status === "sent" && (
//                       <div className="text-right text-xs">
//                         <div>{campaign.recipients > 0 ? `${((campaign.opens / campaign.recipients) * 100).toFixed(1)}% open` : "0% open"}</div>
//                         <div>{campaign.recipients > 0 ? `${((campaign.clicks / campaign.recipients) * 100).toFixed(1)}% click` : "0% click"}</div>
//                       </div>
//                     )}
//                   </div>
//                 ))}
...
//                     onClick={() => fetchCampaigns(pagination.page - 1, pagination.limit)}
//                   >
//                     Previous
//                   </Button>
//                   <Button
...
export default function NewsletterPage() {
  ...
  const newsletterStats: NewsletterStats[] = [
    ...
    },
    {
      title: "Open Rate",
      value: campaigns.length > 0 
        ? `${(campaigns.reduce((sum, c) => sum + (c.opens || 0), 0) / campaigns.reduce((sum, c) => sum + (c.recipients || 0), 1) * 100).toFixed(1)}%`
        : "0%",
      change: "+2.3%",
      trend: "up",
      icon: Eye,
    },
    {
      title: "Click Rate",
      value: campaigns.length > 0 
        ? `${(campaigns.reduce((sum, c) => sum + (c.clicks || 0), 0) / campaigns.reduce((sum, c) => sum + (c.recipients || 0), 1) * 100).toFixed(1)}%`
        : "0%",
      change: "+1.1%",
      trend: "up",
      icon: TrendingUp,
              ...
              <CardContent className="space-y-4">
                {campaigns.slice(0, 3).map((campaign) => (
                  <div key={campaign._id} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    {campaign.status === "sent" && (
                      <div className="text-right text-xs">
                        <div>{(campaign.recipients || 0) > 0 ? `${(((campaign.opens || 0) / (campaign.recipients || 1)) * 100).toFixed(1)}% open` : "0% open"}</div>
                        <div>{(campaign.recipients || 0) > 0 ? `${(((campaign.clicks || 0) / (campaign.recipients || 1)) * 100).toFixed(1)}% click` : "0% click"}</div>
                      </div>
                    )}
                  </div>
                ))}
                ...
                <TableBody>
                  {campaigns.map((campaign) => (
                    <TableRow key={campaign._id}>
                      ...
                      <TableCell>{(campaign.recipients || 0).toLocaleString()}</TableCell>
                      <TableCell>
                        {(campaign.recipients || 0) > 0 
                          ? `${(((campaign.opens || 0) / campaign.recipients) * 100).toFixed(1)}%` 
                          : "0%"
                        }
                      </TableCell>
                      <TableCell>
                        {(campaign.recipients || 0) > 0 
                          ? `${(((campaign.clicks || 0) / campaign.recipients) * 100).toFixed(1)}%` 
                          : "0%"
                        }
                      </TableCell>
                      <TableCell>{campaign.sentAt ? formatDate(campaign.sentAt) : "Not sent"}</TableCell>
