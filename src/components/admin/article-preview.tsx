...
export function ArticlePreview({ article }: ArticlePreviewProps) {
  const formatContent = (content: string) => {
    ...
    return content
      .replace(/### (.*)/g, '<h3 class="text-xl font-semibold mb-3 mt-6">$1</h3>')
      .replace(/## (.*)/g, '<h2 class="text-2xl font-semibold mb-4 mt-8">$1</h2>')
      .replace(/# (.*)/g, '<h1 class="text-3xl font-bold mb-4 mt-8">$1</h1>')
      .replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>")
      .replace(/\*(.*?)\*/g, "<em>$1</em>")
      .replace(/<u>(.*?)<\/u>/g, "<u>$1</u>")
      .replace(/> (.*)/g, '<blockquote class="border-l-4 border-primary pl-4 italic my-4">$1</blockquote>')
      ...
      <Card>
        <CardContent className="p-8">
          ...
          <div className="space-y-4 mb-8">
            {article.layoutConfig.showCategory && <Badge className="mb-2">Technology</Badge>}

            <h1 className="text-4xl font-bold text-foreground leading-tight">{article.title || "Article Title"}</h1>
          ...
          {article.featuredImage && (
            ...
            >
              <img
                src={article.featuredImage || "/placeholder.svg?height=400&width=800&query=news article featured image"}
                alt="Featured"
                className={`rounded-lg ${
                  article.layoutConfig.imagePosition === "left" || article.layoutConfig.imagePosition === "right"
                    ? "w-80"
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: formatContent(article.content || "Your article content will appear here..."),
            }}
          />
            ...
            <div className="border-t border-border pt-6 mb-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">News Contributor</p>
                </div>
